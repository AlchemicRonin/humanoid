...
class MINI_Approx_ZMP_Multi_Mass:
    def __init__(self):
        ...
        for link in self.links:
            if (
                self.links[link].inertial == None
                or self.links[link].inertial.origin == None
            ):
            ...
            for link in self.links:
                try:
                    ...
                    if len(link_info.com_x_pos_queue) > queue_len:
                        ...
                        px_sum += m * ((z_ddot + g) * x - (z - pz) * x_ddot)
                        py_sum += m * ((z_ddot + g) * y - (z - pz) * y_ddot)
                        denom_sum += m * (z_ddot + g)

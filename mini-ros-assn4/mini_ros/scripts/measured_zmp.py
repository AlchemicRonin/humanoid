...
class MINI_Measured_ZMP:
    def __init__(self):
        ...
        RF_FT_sensor = rospy.Subscriber(
            "/RF_FT_sensor", WrenchStamped, self.RF_FT_sensor_callback
        )
        LF_FT_sensor = rospy.Subscriber(
            "/LF_FT_sensor", WrenchStamped, self.LF_FT_sensor_callback
        )
    ...
    def start(self):
        trans_l = self.tfBuffer.lookup_transform(
            "base_link", "l_foot_link", rospy.Time()
        )
        trans_r = self.tfBuffer.lookup_transform(
            "base_link", "r_foot_link", rospy.Time()
        )
        ...
        px = (
            right_zmp.point.x * self.right_force_z
            + left_zmp.point.x * self.left_force_z
        ) / (self.right_force_z + self.left_force_z)
        py = (
            right_zmp.point.y * self.right_force_z
            + left_zmp.point.y * self.left_force_z
        ) / (self.right_force_z + self.left_force_z)
        pz = -0.165

...
class MINI_Approx_ZMP_Single_Mass:
    def __init__(self):
        ...
        self.approx_zmp = rospy.Publisher(
            "/approx_zmp_single_mass", Float32MultiArray, queue_size=10
        )
        self.approx_zmp_marker = rospy.Publisher(
            "/approx_zmp_single_mass_marker", Marker, queue_size=10
        )
    ...
    def calc_single_mass_zmp(
        self, x_pos, y_pos, z_pos, x_accl, y_accl, z_accl, g=9.81, pz=0.30
    ):
        px = 0
        py = 0
    ...
    def com_callback(self, msg):
        ...
        if len(self.com_x_pos_queue) > queue_len:
            ...
            x_vel = [(x_pos[1] - x_pos[0]) / dt1, (x_pos[2] - x_pos[1]) / dt2]
            x_accl = x_vel[1] - x_vel[0] / dt2

            y_vel = [(y_pos[1] - y_pos[0]) / dt1, (y_pos[2] - y_pos[1]) / dt2]
            y_accl = y_vel[1] - y_vel[0] / dt2

            z_vel = [(z_pos[1] - z_pos[0]) / dt1, (z_pos[2] - z_pos[1]) / dt2]
            z_accl = z_vel[1] - z_vel[0] / dt2

            if math.isnan(z_accl):
                return

            zmp_x, zmp_y = self.calc_single_mass_zmp(
                x_pos[0], y_pos[0], z_pos[0], x_accl, y_accl, z_accl
            )
